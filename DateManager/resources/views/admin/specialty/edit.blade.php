...
@section('title')
    {{__('Actualización de especialidad')}}
@endsection

@section('content')
    <div class="row justify-content-center">
        <div class="col-7 card shadow-lg mt-5 p-5 rounded">
            <form class="row justify-content-center" action="{{ route('specialty.update',$specialty) }}" method="post">
                @method('PUT')
                @csrf
                ...
                <div class="form-group col-10">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror"  name="description" cols="30" rows="10">{{ old('description',$specialty->description) }}</textarea>
                    @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
